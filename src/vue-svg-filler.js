export default {
  ...
  methods: {
    createSvgElement() {
      ...
      const source = this.path.match(/^http(s?):\/\//)
        ? this.path
      ...
      request.onload = () => {
        if (request.status >= 200 && request.status < 400) {
          ...
          const elementSvg = domParser.parseFromString(
          ...
          if (!pathOfSvg) {
            this._errorLog(
              `No svg path element in your svg file.\nPath : ${source}`
            );
            return;
          }
    ...
    updateSrcSvgElement(val) {
      val
        ? (this.dataOfPath = val)
        : this._errorLog(`Can't get attribute 'd' from your SVG file.`);
    },
    updateFillColor(val) {
      this.fillColor = val;
    },
    ...
    mouseHoverEvent(event) {
      if (event === "over") {
        ...
        if (this.hoverStrokeColor)
          this.updateStrokeColor(this.hoverStrokeColor);
      } else {
